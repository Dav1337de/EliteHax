<?php
	...
	try {
		...
		if (($my_role > 3) or ($member_role == 1) or ($member_role == 5)) { exit("An Error occured!"); }
		//Same Role or less
		if ($my_role >= $member_role) { exit("An Error occured!"); }
		
		if ($member_role == 2) { $role_name = "Expert"; }
		elseif ($member_role == 3) { $role_name = "Hacker"; }
		elseif ($member_role == 4) { $role_name = "Rookie"; }
		...
		//Add Message to logs
		$stmt = $db->prepare("INSERT INTO crew_logs (type,subtype,crew_id,field1,field2,field3,timestamp) VALUES ('action','demote',?,?,?,?,NOW())");
