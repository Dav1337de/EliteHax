<?php
	...
	try {
		...
		$stmt = $db->prepare("SELECT task.*,NOW() as nowtime,TIMESTAMPDIFF(SECOND,NOW(),(SELECT overclock.oc_end FROM overclock WHERE overclock.user_id=?)) as oc_interval,ADDDATE(starttime,INTERVAL (FLOOR(TIMESTAMPDIFF(SECOND,starttime,(SELECT oc_start FROM overclock WHERE user_id=?)))+FLOOR(TIMESTAMPDIFF(SECOND,(SELECT oc_start FROM overclock WHERE user_id=?),endtime)/2)) SECOND) as new_end_time, ADDDATE(starttime, INTERVAL FLOOR(duration/2) SECOND) as new_end_time2 FROM task where id=? ORDER BY endtime DESC LIMIT 1");
		$stmt->bindValue(1, $id, PDO::PARAM_INT);
		$stmt->bindValue(2, $id, PDO::PARAM_INT);
		$stmt->bindValue(3, $id, PDO::PARAM_INT);
		...
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			if ($row['oc_interval'] == null) { $oc_interval=0; }
			else { $oc_interval = $row['oc_interval']; }
			$resp = $resp."{\"secs\": ".$interval.",\n"
			."\"oc_secs\": ".$oc_interval."\n,"
			."}";
		}
