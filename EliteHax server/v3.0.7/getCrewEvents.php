<?php
	...
	try {
		...
		$stmt = $db->prepare("SELECT type,subtype,field1,field2,field3,DATE_SUB(timestamp,INTERVAL 1 HOUR) as timestamp FROM crew_logs WHERE crew_id=? and type='action' ORDER BY timestamp DESC LIMIT 50"); 
		$stmt->bindValue(1, $crew_id, PDO::PARAM_INT);
		$stmt->execute();

		$resp = $resp."\"crew_events\":[\n";
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {	
