<?php
	...
	try {
		...
		$stmt = $db->prepare("SELECT hack_scenario_services.host_id FROM `hack_scenario_services` JOIN hack_scenario_hosts ON hack_scenario_services.host_id=hack_scenario_hosts.id WHERE hack_scenario_services.id=? and hack_scenario_hosts.user_id=?");
		...
		//Insert action with 2 Minutes delay
		$stmt = $db->prepare("INSERT INTO hack_scenario_activities (activity, user_id, service_id, description, start_time, end_time) VALUES (7,?,?,'Service Fingerprint',NOW(),DATE_ADD(NOW(),INTERVAL 2 MINUTE))");
		$stmt->bindValue(1, $id, PDO::PARAM_INT);
