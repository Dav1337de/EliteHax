<?php
	...
	try {
		...
		//Insert action with 5 Minutes delay
		$stmt = $db->prepare("INSERT INTO hack_scenario_activities (activity, user_id, net_id, description, start_time, end_time) VALUES (1,?,?,'Network Host Scan',NOW(),DATE_ADD(NOW(),INTERVAL 5 MINUTE))");
		$stmt->bindValue(1, $id, PDO::PARAM_INT);
		$stmt->bindValue(2, $net_id, PDO::PARAM_INT);
		$stmt->execute();
		
		$stmt = $db->prepare("UPDATE hack_scenario_networks SET host_scan=1 WHERE id=? and user_id=?");
