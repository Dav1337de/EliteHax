<?php
	...
	try {
		...
		//Player Tables
		$stmt = $db->prepare("DROP TABLE IF EXISTS tournament_hack_finish");		
		$stmt->execute();
		$db->beginTransaction();
		$stmt = $db->prepare('SET @curRank := 0;');
		$stmt->execute();
		$stmt = $db->prepare("CREATE TABLE tournament_hack_finish SELECT rank,id,username, crew, money_hack, hack_count FROM (SELECT id,username,money_hack,hack_count,crew,@curRank := @curRank + 1 AS rank FROM (SELECT tournament_hack.* FROM `tournament_hack` JOIN user ON tournament_hack.id=user.id WHERE hack_count>0 and money_hack>0 order by money_hack DESC, hack_count ASC LIMIT 1000) as t) as t2");		
		$stmt->execute();
		$db->commit();
		
		//Crew Tables
		$stmt = $db->prepare("DROP TABLE IF EXISTS tournament_hack_finish_crew");		
		$stmt->execute();
		$db->beginTransaction();
		$stmt = $db->prepare('SET @curRank := 0;');
		$stmt->execute();
		$stmt = $db->prepare("CREATE TABLE tournament_hack_finish_crew SELECT t.*,@curRank := @curRank + 1 AS rank FROM (SELECT t2.* FROM (SELECT tournament_hack.crew,SUM(money_hack) as total,crew.name,crew.tag FROM `tournament_hack` JOIN crew ON tournament_hack.crew=crew.id WHERE crew <> 0 GROUP BY tournament_hack.crew HAVING sum(money_hack)>0 ORDER BY sum(money_hack) DESC LIMIT 1000) as t2 JOIN user ON t2.crew=user.crew GROUP BY user.crew HAVING count(user.crew)>2 ORDER BY total DESC) as t");		
		$stmt->execute();
		$db->commit();
		
		//Player Rewards
		$stmt = $db->prepare("SELECT tournament_hack_finish.*,skill_tree.lvl,skill_tree.xp,user_stats.tournament_best,user_stats.tournament_won FROM tournament_hack_finish JOIN skill_tree ON tournament_hack_finish.id=skill_tree.user_id JOIN user_stats ON skill_tree.user_id=user_stats.id");		
		$stmt->execute();
		...
		//Crew Rewards
		$stmt = $db->prepare("SELECT tournament_hack_finish_crew.*,count(user.id) as members,crew.tournament_best FROM tournament_hack_finish_crew JOIN user ON tournament_hack_finish_crew.crew=user.crew JOIN crew ON user.crew=crew.id GROUP BY (user.crew) ORDER BY rank");		
		$stmt->execute();
		$crews=$stmt->rowCount();
		if ($crews>0) {
			$arr = $stmt->fetchAll(PDO::FETCH_ASSOC);
			foreach ($arr as $row) {
				...
				$stmt->execute();
				//Add Message to logs
				$stmt = $db->prepare("INSERT INTO crew_logs (type,subtype,crew_id,field1,field2,field3,timestamp) VALUES ('tournament','hack',?,?,?,?,NOW())");
				$stmt->bindValue(1, $crew, PDO::PARAM_INT);
