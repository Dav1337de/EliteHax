...
local missionTutScene = composer.newScene()
local tutPage=1
---------------------------------------------------------------------------------
...
local close = function(event)
    if (event.phase == "ended") then
        if (tutPage==1) then
            myData.instruction.text="\nThe duration and reward of your missions depend on your GPU and your Mission Center levels.\n\nTap the Start button to start a mission!"
            tutPage=2
        elseif (tutPage==2) then
            myData.instruction.text="\nWhen the Mission timer is ended, the Collect button will appear and you can collect your rewards!\nWhen you collect all the available missions, you will receive new missions."
            tutPage=3
        elseif (tutPage==3) then
            myData.instruction.text="\n\nUpgrade your Mission Center to receive better missions and run multiple missions at the same time."
            tutPage=4
        ...
        elseif (tutPage==4) then
            ...
            loadsave.saveTable( tutorialStatus, "missionTutorialStatus.json" )
            tutOverlay = false
...
---------------------------------------------------------------------------------
--  Scene Creation
function missionTutScene:create(event)
    tutGroup = self.view
    ...
    local options = 
    {
        text = "\n\nIn the Mission screen you can start missions, collect rewards and upgrade your Mission Center.",     
...
-- Home Show
function missionTutScene:show(event)
...
--> Listener setup
---------------------------------------------------------------------------------
missionTutScene:addEventListener( "create", missionTutScene )
missionTutScene:addEventListener( "show", missionTutScene )
---------------------------------------------------------------------------------

return missionTutScene