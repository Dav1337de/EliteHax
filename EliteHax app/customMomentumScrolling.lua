...
local function setLimits( self, view )
	...
	-- Set the left limit
	view._scrollWidth=view._scrollHeight
	if view._scrollWidth then

		leftLimit = ( - view._scrollWidth + view._height ) - view._rightPadding
		if isGraphicsV1 then
			leftLimit = leftLimit - view._height * 0.5
		end
		leftLimit = leftLimit
	end
	...
	-----
	rightLimit=0
	leftLimit=(-view._scrollWidth+view._height)
	...
	if not view._isHorizontalScrollingDisabled then
		...
		if view.y < leftLimit then
			...
			if "boolean" == type( snapBack ) then
				if snapBack == true then
					if view._tween then transition.cancel( view._tween ) end
					view._tween = transition.to( view, { time = bounceTime, y = leftLimit, transition = easing.outQuad, onComplete = onBounceComplete } )
					
				end
			end
		
		-- Put the view back to the right if it isn't already there ( and should be )
		elseif view.y > rightLimit then
			-- Set the hit limit
			limitHit = "right"
			...
			if "boolean" == type( snapBack ) then
				if snapBack == true then
					if view._tween then transition.cancel( view._tween ) end
					view._tween = transition.to( view, { time = bounceTime, y = rightLimit, transition = easing.outQuad, onComplete = onBounceComplete } )
				end
			end
		end
	end
